...
namespace PW.Ncels.Database.Repository.OBK
{
    public class OBKContractRepository : ARepository
    {
        ...
        {
            var employeeId = UserHelper.GetCurrentEmployee().Id;
            var list = AppContext.OBK_Contract.Where(x => x.ParentId == null && x.EmployeeId == employeeId && x.Status != CodeConstManager.STATUS_OBK_ONCORRECTION && x.Status != CodeConstManager.STATUS_OBK_WORK && x.Status != CodeConstManager.STATUS_OBK_INPROCESSING && x.Status != CodeConstManager.STATUS_OBK_DRAFT_ID).Select(x => new { Id = x.Id, Name = x.Number }).ToList();
            return list;
        }
